...
export class ReservationsService {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (conflict) {
        throw new ConflictException(
          `Room is occupied ${conflict.start.toISOString()} to ${conflict.end.toISOString()}`,
        );
      }
  ...
  async cancel(id: string, userId: bigint, role: string) {
    ...
    if (
      !isAdmin &&
      hoursBeforeStart < reservationConstants.CANCELLATION_DEADLINE_HOURS
    )
      throw new BadRequestException(
        `Cancellation must be made at ${reservationConstants.CANCELLATION_DEADLINE_HOURS} hour(s) in advance.`,
      );
